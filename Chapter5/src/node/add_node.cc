...
Node *AddNode::idealize() {
    ...
    if (i && i->value() == 0) return lhs;
    // Pointer comparison?

    // Add of same to a multiply by 2
    if (lhs == rhs) {
        return new MulNode(lhs, (new ConstantNode(TypeInteger::constant(2), Parser::START))->peephole());
    }
...
bool AddNode::spline_cmp(Node *hi, Node *lo) {
    if (lo->type_->isConstant()) return false;
