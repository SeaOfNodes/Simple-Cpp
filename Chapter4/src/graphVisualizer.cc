...
void GraphVisualizer::nodes(std::ostringstream &sb,
    ...
    for (Node *n: all) {
        ...
        if (dynamic_cast<MultiNode *>(n)) {
            ...
            sb << "\t\t\t<TABLE BORDER=\"0\" CELLBORDER=\"1\" CELLSPACING=\"0\" CELLPADDING=\"4\">\n";
            ...
            for (auto *use: n->outputs) {
                if (auto *proj = dynamic_cast<ProjNode *>(use)) {
                    ...
                    if (proj->isCFG()) sb << " BGCOLOR=\"yellow\"";
                    sb << ">" << proj->glabel() << "</TD>\n";
                }
            }
            if (doProjTable) {
...
void GraphVisualizer::nodeEdges(std::ostringstream &sb,
    ...
    for (Node *n: all) {
        ...
        if (dynamic_cast<ProjNode *>(n) || dynamic_cast<ScopeNode *>(n) || dynamic_cast<ProjNode *>(n)) continue;
        int i = 0;
...
void GraphVisualizer::scopes(std::ostringstream &sb, ScopeNode *n) {
    ...
    for (auto &scope: n->scopes) {
        ...
        for (const auto &pair: scope) {
            ...
            sb << "<TD PORT=\"" << makePortName(scopeName, name) << "\">" << name << "</TD>";
        }
...
std::string GraphVisualizer::makePortName(std::string ScopeName, std::string varName) {
