...
TypeStruct* TypeStruct::ARY() {
    ...
    return make_Ary(TypeInteger::BOT(), -1, TypeInteger::BOT(), -2);
}
...
TypeStruct* TypeStruct::S1() {
    ...
    fields.push_back(Field::make("s2", TypeMemPtr::make(2, S2F()), -2,  false));
...
TypeStruct* TypeStruct::S2() {
    Tomi::Vector<Field*> fields;
    fields.push_back(Field::make("S1", TypeMemPtr::make(2, S1F()), -3, false));
    fields.push_back(Field::make("b", TypeFloat::F64(), -4, false));
...
std::ostringstream &TypeStruct::gprint(std::ostringstream& builder) {
    builder << name_;
    return builder;
}
