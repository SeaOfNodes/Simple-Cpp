...
bool Type::isHighOrConst() { return isHigh() || isConstant();}
bool Type::isConstant() { return type_ == TNIL(); }
...
Type *Type::NIL() {
    return alloc.new_object<Type>(TNIL)->intern();
}
Type* Type::XNIL() {
    return alloc.new_object<Type>(TXNIL)->intern();
}
int Type::log_size() {
...
std::ostringstream& Type::gprint(std::ostringstream& builder) {
...
Type *Type::meet(Type *other) {
  ...
  // TypeNil vs TypeNil meet
    auto*ptr0 = dynamic_cast<TypeNil*>(this)
    auto*ptr1 = dynamic_cast<TypeNil*>(other);
...
Type *Type::xmeet(Type *t) {
  ...
  if (type_ == TTOP || t->type_ == TBOT)
    ...
    if (type_ == TNIL) {
        if (auto ptr = dynamic_cast<const TypeNil*>(t)) {
            return ptr->meet0();
        } else {
            return (t->type_ == TXNIL) ? TypePtr::PTR() : BOTTOM();
        }
    }
    if (type_ == TXNIL) {
        if (auto ptr = dynamic_cast<const TypeNil*>(t)) {
            ...
            return (t->type_ == TNIL) ? TypePtr::PTR() : BOTTOM();
        }
    }
