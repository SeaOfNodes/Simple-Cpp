...
TypeMem *TypeMem::BOT() {
    static TypeMem *bot = make(0, Type::BOTTOM());
    return bot;
}

void TypeMem::gather(Tomi::Vector<Type *> &ts) {
    ts.push_back(make(1, Type::NIL()));
...
TypeMem *TypeMem::xmeet(Type *t) {
    ...
    int alias = alias_ == that->alias_ ? alias_ : 0;
...
std::ostringstream &TypeMem::gprint(std::ostringstream &builder) {
    builder << "#";
    if(alias_ ==0)  builder << (t_->type_ == TTOP() ? "TOP" : "BOT");
    else if(alias_ == -1) builder << "BOT";
    builder << alias_;
    builder << ":";
    return t_->gprint();
}
