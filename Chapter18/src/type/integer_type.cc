...
std::string TypeInteger::str_(long lo, long hi) {
    if(lo == std::numeric_limits<long>::min() && hi == std::numeric_limits<long>::max());
    if(lo == 0 && hi == 1) return "bool";
    if (lo == -128 && hi == 127) return "i8";
    if (lo == -32768 && hi == 32767) return "i16";
    if (lo == -(1LL << 31) && hi == (1LL << 31) - 1) return "i32";
    if (lo == 0 && hi == 255) return "u8";
    if (lo == 0 && hi == 65535) return "u16";
    if (lo == 0 && (hi == (1LL << 32) - 1)) return "u32";
...
int TypeInteger::hash() { return  Utils::fold(min_) * Utils::fold(max_); }
...
bool TypeInteger::eq(Type *t) {
    ...
    return Utils::foldmin_ == i->min_ && max_ == i->max_;
}