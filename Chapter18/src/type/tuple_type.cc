...
std::ostringstream& TypeTuple::gprint(std::ostringstream &builder) {
    if(this == TOP()) {
        builder << "[TOP]";
        return builder;
    }
...
Type *TypeTuple::xmeet(Type *other) {
  ...
  if(this == TypeTuple::TOP()) return other;
  if(tt == TypeTuple::TOP()) return this;

  if(types_.size() != tt->types_.size()) {
      return  TypeTuple::BOT();
  }
  Tomi::Vector<Type *> ts;
  int cnt = 0;
...
TypeTuple *TypeTuple::dual() {
    if(this == TypeTuple::TOP()) return TypeTuple::BOT();
    if(this == TypeTuple::BOT()) return TypeTuple::TOP();
    Tomi::Vector<Type *> ts(types_.size());
    for(int i = 0; i < types_.size(); i++) {
        ts.push_back(types_[i]->dual());
    }
    return make(ts);
}
...
std::ostringstream &TypeTuple::print_1(std::ostringstream &builder) {
 ...
 int TypeTuple::hash() {
   int sum = 0;
   // types cant be null, so it will be empty and move to sum
   for (Type *type : types_) {
    sum ^= type->hashCode();
   }
  return sum;
...
bool TypeTuple::eq(Type *t) {
  auto *tt = (TypeTuple *)(t);
  if(types_null && tt->types_null) return true;
  if(types_null || tt->types_null) return false;
...
// types are null
TypeTuple *TypeTuple::TOP() {
    static TypeTuple TOP(Tomi::Vector<Type*>{});
    TOP->types_null = true;
    return dynamic_cast<TypeTuple*>(TOP.intern());
}
// types are empty
TypeTuple *TypeTuple::BOT() {
    static TypeTuple BOT(Tomi::Vector<Type*>{});
    return dynamic_cast<TypeTuple*>(TOP.intern());
}
TypeType* TypeTuple::MAIN() {
    return make(TypeInteger::BOT());
}

TypeType* TypeTuple::RET() {
    return make({Type::CONTROL(), TypeMem::BOT(), Type::BOTTOM()});
}
