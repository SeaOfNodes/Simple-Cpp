...
RegionNode::RegionNode(Lexer *loc, std::initializer_list<Node *> nodes) : CFGNode(nodes), loc_(loc) {}
...
Node *RegionNode::idealize() {
    ...
    if (!hashPhi() && p1 && p2 && p1->in(0)->addDep(this) == p2->in(0)->addDep(this) &&
        dynamic_cast<IfNode *>(p1->in(0))) {
